...
public abstract class Military extends Robot {
    ...
    public void CreateRobot(Scanner scanner) {
        ...
        if (scanner.next().charAt(0)=='y')this.setArmor(true);
        else this.setArmor(false);
        if (this.isArmor()) {
            System.out.println("Armor Level from 1 to 5");
            this.setArmorLevel(scanner.nextInt());
        }
        System.out.println("has camo?yes or no(or y/n)");
        if (scanner.next().charAt(0)=='y')this.setCamouflagePaint(true);
        else this.setCamouflagePaint(false);
        if (this.isCamouflagePaint()){
            System.out.println("Enter Camo name");
            this.setCamouflagePaintName(scanner.next());
        }
        System.out.println("Army type");
        this.setArmyType(scanner.next());
    }
}
